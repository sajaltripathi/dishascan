...
Rebuilt/upgraded source — not a new app. Package is now `com.dishascan.app` — renamed twice: `com.vastusurvey.camera` → `com.tiscan.app` → `com.dishascan.app` (TiScan conflicted with an existing app on Google Play). Since Android treats package name as an app's identity, this installs as a fresh app, not an update over any earlier build on your phone.
...
`dishascan-debug`**. That zip contains `app-debug.apk` — a real,
