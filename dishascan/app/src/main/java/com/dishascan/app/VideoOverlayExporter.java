...
public class VideoOverlayExporter {
    ...
    public static void export(final Context c, Uri src, final List<Sample> ss, final File tmp, final Listener l) {
        try {
            TextOverlay txt = new TextOverlay() { // from class: com.dishascan.app.VideoOverlayExporter.1
            ...
            OverlayEffect oe = new OverlayEffect(ImmutableList.<TextureOverlay>of(txt));
            Effects ef = new Effects(ImmutableList.of(), ImmutableList.<androidx.media3.common.Effect>of(oe));
            EditedMediaItem item = new EditedMediaItem.Builder(MediaItem.fromUri(src)).setEffects(ef).build();
            if (tmp.exists()) {
                tmp.delete();
            }
            Transformer t = new Transformer.Builder(c).addListener(new Transformer.Listener() { // from class: com.dishascan.app.VideoOverlayExporter.2
